...
public interface AvailabilityLogRepository extends JpaRepository<AvailabilityLog, Long> {
    ...
    @Query(value = "SELECT strftime('%Y-%m-%d', timestamp) as day, COUNT(id) FROM availability_logs WHERE status_value = 0 GROUP BY day ORDER BY day ASC", nativeQuery = true)
